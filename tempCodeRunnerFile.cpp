...
void searchbooktitle()
 {
    ...
    for (int i = 0; i < nbooks; i++)    
    {
        if(sbook==books[i].title) 
        {
            ...
            cout << i + 1 << ". " << books[i].title << " by " << books[i].author << " (Quantity: " << books[i].qty << ")" <<" ISBN:"<<books[i].ISBN<< endl;
            found = true;
        }
    }
    if (!found) {
...
{
    int id;
    cout<<" Enter borrower ID you want to search:";
    cin>>id;
    bool found = false;
    for (int i = 0; i < nborrowers; i++)
    {
...
{
    string bname;
    cout<<" Enter borrower name you want to search:";
    cin.ignore();
    getline(cin,bname);
    bool found = false;
    for (int i = 0; i < nborrowers; i++) 
